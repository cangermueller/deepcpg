...
h <- dat$act %>% select(-c(target, n)) %>%
  filter(!is.na(rs), act_mean > 0, act_std > 0) %>%
...
for (f in dat$tab$filt) {
  ...
  h <- ftab %>%
    select(label, act_mean, act_std, rs, rs_pvalue, nb_hits,
      Target.ID, Target.name, name, q.value, URL) %>%
    mutate(sel=(act_mean >= 0.001 && abs(rs) >= 0.1))
  print(xtable(h, digits=3, include.rownames=F))
  cat('<table>')
  cat('<tr>')
