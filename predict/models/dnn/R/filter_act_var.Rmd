...
```{r}
opts_chunk$set(fig.width=9, fig.height=max(10, cmp$nb_sel * 0.25))
```
...
## Spearman variance - mean
```{r}
p <- plot_heat_del(dat$anno)
```

## Spearman
```{r}
d <- dat$annos %>% filter(cell_type=='serum', wlen==3000)
p <- plot_heat_annos(d %>% filter(metric=='var'), 'rs')
p <- plot_heat_annos(d %>% filter(metric=='mean'), 'rs',
