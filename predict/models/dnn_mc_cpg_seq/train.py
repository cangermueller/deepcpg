...
from callbacks import PerformanceLogger
...
def add_seq_conv(model, params, prefix='s'):
    ...
    model.add_node(input=lab('f1'), name=lab('f1d'), layer=kcore.Dropout(params.dropout))
    # hidden
    ...
    model.add_node(input=lab('h1'), name=lab('h1d'), layer=kcore.Dropout(params.dropout))
...
def add_cpg_conv(model, params, prefix='c'):
    ...
    model.add_node(input=lab('f1'), name=lab('f1d'), layer=kcore.Dropout(params.dropout))
    # hidden
    nconv = params.dim[1] * (params.dim[0] // params.pool_len) * params.num_filters
    layer = kcore.Dense(nconv, params.num_hidden,
                    activation='relu',
                    init='glorot_uniform')
    model.add_node(input=lab('f1d'), name=lab('h1'), layer=layer)
